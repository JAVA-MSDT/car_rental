...
public class AuthController implements Filter{
	...
	private static final List<String> PUBLIC_CONTROLLER = Arrays.asList(
			ControllerConstant.BLOG_CONTROLLER,
			ControllerConstant.ABOUT_CONTROLLER,
			ControllerConstant.NEWS_CONTROLLER,
			ControllerConstant.CARS_CONTROLLER,
			ControllerConstant.LOGIN_CONTROLLER,
			ControllerConstant.LOGOUT_CONTROLLER
			);
	
	private static final List<String> ADMINISTRATION_CONTROLLER = Arrays.asList(
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
		...
		System.out.println("Controller Filter:: " + controller);
        if(!isPublicController(controller)) {
        	if(user.getUseRole().equals(UserRole.ADMIN) && isAdministrationController(controller)) {
        		filterChain.doFilter(request, response);
        	} else if (user.getUseRole().equals(UserRole.USER) && isUserController(controller)) {
        		filterChain.doFilter(request, response);
			}else {
	...
	/**
     * @param contoroller to be checked if the administration authorized to access it or not
    ...
    /**
     * @param command to be checked if the user authorized to access on of this command or not
